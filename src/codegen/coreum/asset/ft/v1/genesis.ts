...
export const GenesisState = {
  ...
  fromAmino(object: GenesisStateAmino): GenesisState {
    ...
    message.frozenBalances = object.frozen_balances?.map(e => Balance.fromAmino(e)) || [];
    message.whitelistedBalances = object.whitelisted_balances?.map(e => Balance.fromAmino(e)) || [];
    message.pendingTokenUpgrades = object.pending_token_upgrades?.map(e => PendingTokenUpgrade.fromAmino(e)) || [];
    return message;
  },
...
export const PendingTokenUpgrade = {
  typeUrl: "/coreum.asset.ft.v1.PendingTokenUpgrade",
  encode(message: PendingTokenUpgrade, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    if (message.denom !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PendingTokenUpgrade {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  fromPartial(object: Partial<PendingTokenUpgrade>): PendingTokenUpgrade {
    const message = createBasePendingTokenUpgrade();
